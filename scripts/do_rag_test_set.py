...
class Config:
    TEST_SET_PATH = os.path.abspath(f"{script_dir}/../data/test_set.json")
    LOAD_FROM_CHECKPOINT = False
    if LOAD_FROM_CHECKPOINT:
        # introduce manually the checkpoint to run from
        CHECKPOINT_PATH = os.path.abspath(f"{script_dir}/../output/checkpoints/rag_XXX.json") 
    else:
...
for i, qa in enumerate(tqdm(test_set, desc="Procesando preguntas")):
    ...
    try:
        start_time = time()
        generated_answer, retrieved_contexts, used_tokens = rag_pipeline.do_rag(query=query)
        end_time = time()
    ...
    qa_result = {**qa, 
                 ...
                 "time_taken_seconds": end_time - start_time if generated_answer else None,
                 "input_tokens": used_tokens["input_tokens"] if generated_answer else None,
                 "output_tokens": used_tokens["output_tokens"] if generated_answer else None,
    }
